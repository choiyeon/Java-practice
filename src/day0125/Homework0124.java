...
/**
 * 숙제. - InputDialog를 사용하여 아래와 같은 동작을 수행하는 프로그램을 작성하세요.
 * 
 ...
 * 
 * 
 * 2. 위의 Dialog에서 “1”이 입력 되면 학생번호,이름,자바,오라클 점수를 입력 받을 수 있는 InputDialog를 제공한다.
 * CSV형식의 데이터를 입력받아, 학생의 번호가 존재한다면 “이미 존재하는 번호입니다.” 를 MessageDialog에 보여주세요. 그렇지
 * 않으면 VO에 입력.
 * 
 * 
 * 
 * 
 * 
 * 3.1번 메뉴에서 “2”가 입력되면 저장된 모든 값을 console에 출력하는 코드 작성 번호 이름 자바점수 오라클점수 합계 평균 1
 * xxx 90 90 90 90.xx
 * 
...
//				case 1:	// 입력 데이터가 한 자인 경우
//					switch(parseInt(inputData)) {
//					case INPUTD_DATA:
//						inputData();
//						break;
//					case PRINT_DATA:
		...
		if (inputData != null) {
			...
			case NUM_OF_INDEX :
				...
				if (!map.containsKey(num)) {
					map.put(num, new WorkVO(num, arr[NAME], parseInt(arr[SCORE_JAVA]),
							parseInt(arr[SCORE_ORACLE])));
				} else {
					JOptionPane.showMessageDialog(null, num + "번은 이미 존재하는 번호입니다");
				} // end else
			}
		} // end if
...
//		for (int i = 0; i < arr.length; i++) {
//			wVO = map.get(arr[i]);
//			sb.append(wVO.getNum()).append("\t").append(wVO.getName()).append("\t").append(wVO.getScoreJava())
//					.append("\t\t").append(wVO.getScoreOracle()).append("\t\t")
//					.append(wVO.getScoreJava() + wVO.getScoreOracle()).append("\t")
//					.append(df.format((wVO.getScoreJava() + wVO.getScoreOracle()) / 2.0)).append("\n");
//		} // end for
			//
